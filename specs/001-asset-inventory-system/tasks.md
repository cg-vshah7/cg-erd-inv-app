...
- [ ] T042 [P] Create `backend/app/schemas/customer_account.py`: Pydantic v2 `CustomerAccountCreate`, `CustomerAccountUpdate`, `CustomerAccountRead` (with `id`, `created_at`), `CustomerAccountList`
- [ ] T043 [P] Create `backend/app/schemas/engineer.py`: `EngineerCreate` (email, full_name, password, is_super_admin), `EngineerUpdate`, `EngineerRead`, `EngineerList`; `backend/app/schemas/engineer_account_mapping.py`: `MappingCreate` (account_id, can_manage_models, can_checkin_out, can_view_only), `MappingRead`, `MappingUpdate`
- [ ] T044 [P] Create `backend/app/schemas/common.py`: `PaginatedResponse[T]` generic (items, total, skip, limit), `ErrorResponse` (code, message, details)
- [ ] T045 [P] Create `backend/app/repositories/customer_account_repo.py` extending `BaseRepository[CustomerAccount]`; add `get_by_name(name) → CustomerAccount | None`
- [ ] T046 [P] Create `backend/app/repositories/engineer_repo.py` extending `BaseRepository[Engineer]`; add `get_by_keycloak_id(kc_id)`, `get_by_email(email)`, `get_with_mappings(engineer_id)`
- [ ] T047 Create `backend/app/api/v1/accounts.py`: Super Admin only CRUD — `GET /accounts` (paginated), `POST /accounts`, `GET /accounts/{id}`, `PATCH /accounts/{id}`, `DELETE /accounts/{id}` (soft-delete via `is_active=False`)
- [ ] T048 Create `backend/app/api/v1/engineers.py`: Super Admin only — `GET /engineers`, `POST /engineers` (creates KC user + DB row), `GET /engineers/{id}`, `PATCH /engineers/{id}`, `GET /engineers/{id}/accounts`, `POST /engineers/{id}/accounts`, `PATCH /engineers/{id}/accounts/{account_id}`, `DELETE /engineers/{id}/accounts/{account_id}`
- [ ] T049 Create `backend/app/api/v1/auth.py`: `GET /auth/me` returns current engineer profile; `POST /auth/password-reset/request` triggers `KeycloakAdmin.reset_password_email()`
- [ ] T050 Create `backend/app/api/v1/router.py` importing and including all sub-routers under prefix `/api/v1`; update `backend/app/main.py` to include this router
- [ ] T051 [P] Create `frontend/src/hooks/useAccounts.ts`: React Query hooks `useAccounts()`, `useAccount(id)`, `useCreateAccount()`, `useUpdateAccount()` using `/api/v1/accounts`
- [ ] T052 [P] Create `frontend/src/hooks/useEngineers.ts`: hooks `useEngineers()`, `useEngineer(id)`, `useCreateEngineer()`, `useUpdateEngineer()`, `useEngineerAccounts(id)`, `useAssignAccount()`, `useUpdateMapping()`, `useRemoveMapping()`
- [ ] T053 [P] Create `frontend/src/pages/AccountsPage.tsx`: DataTable of customer accounts with columns (name, status, contact); Add/Edit dialogs (MUI Dialog + form); Super Admin only (wrapped in SuperAdminRoute)
- [ ] T054 [P] Create `frontend/src/pages/EngineersPage.tsx`: DataTable of engineers; Create dialog (email, full_name, password, is_super_admin); account mapping panel (assign accounts + permission checkboxes); Super Admin only
...
- [ ] T072 Create `backend/app/schemas/audit_log.py`: `AuditLogRead` (device serial, action, engineer name, event_at, location_snapshot, comments), `AuditLogListParams` (device_id, account_id, engineer_id, action, from_date, to_date, skip, limit)
- [ ] T073 Create `backend/app/repositories/device_repo.py`: `get_checked_in_by_serial(serial, account_id)` (duplicate guard), `get_scoped_devices(engineer_accounts: list[UUID], params: DeviceListParams) → (list[Device], int)` (applies all filters, trigram `serial_number` search via `serial_number ILIKE %term%` or `similarity()`)
- [ ] T074 Create `backend/app/repositories/audit_log_repo.py`: `insert(log_data) → AuditLog` only — no update/delete; `get_scoped_logs(engineer_accounts, params) → (list[AuditLog], int)`
- [ ] T075 Create `backend/app/services/device_service.py` with `check_in(request, engineer, db) → Device`:
...
- [ ] T076 Create `backend/app/api/v1/devices.py` initial routes: `POST /devices/checkin` (calls `device_service.check_in`), `GET /devices` (paginated list, scoped to engineer's accounts), `GET /devices/{id}` (scoped)
- [ ] T077 Create `frontend/src/hooks/useDevices.ts`: `useDevices(params)`, `useDevice(id)`, `useCheckIn()` mutation (invalidates devices query on success)
- [ ] T078 Create `frontend/src/pages/CheckInPage.tsx`: MUI Stepper (3 steps):
